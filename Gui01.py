...
tk.Label(wind, textvariable=var1, bg='light blue', font=('Arial', 17),\
        width=25, height=2).place(x=430,y=100)
...
def do_it():
    ...
    print(InputPromise)
    value = Rm.Rules_matcher(InputPromise, RulesPath = rpath) 
    print(value)   
    var1.set(value)
...
def add_it():
    ...
    newrule = {cl : sp}
    print(newrule)
    Rm.Rules_adder(RulesPath = rpath, NewRule=newrule)
...
def del_it():
    ...
    newrule = {cl : sp}
    print(newrule)
    Rm.Rules_deleter(RulesPath = rpath, OldRule=newrule)
...
tk.Label(wind, text="添加特征", bg='light green', font=('Arial', 13),\
        width=10, height=1).place(x=430,y=200)
...
if __name__ == '__main__':  
    rpath = r'D:\rubbi\BUAA08-AI-DL-HMWRK1-main\BUAA08-AI-DL-HMWRK1-main\Rules.xlsx'
    InputPromise = {'有羽毛', '会游泳', '有黑白二色', '不会飞'}
    testr = {'会游泳', '有黑白二色', '不会飞'}
    print(Rm.Premises(rpath))
    Rm.Rules_matcher(InputPromise, RulesPath = rpath)     
    Rm.Rules_matcher(testr, RulesPath = rpath)    
    newrule = {"不是可食用动物":{"不能吃","不好吃"}} 
    Rm.Rules_adder(RulesPath = rpath, NewRule=newrule)
    print(Rm.Premises(rpath))
    newrule = {"不是可食用动物":{"不能吃","不好吃"}} 
    Rm.Rules_deleter(RulesPath = rpath, OldRule=newrule)
